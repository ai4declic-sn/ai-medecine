...
from langchain.text_splitter import (
    ...
    NLTKTextSplitter,  # python -m nltk.downloader all
)
from langchain.vectorstores import FAISS
from PyPDF2 import PdfReader
from transformers import SpeechT5ForTextToSpeech, SpeechT5HifiGan, SpeechT5Processor, pipeline

from ChatTemplate import bot_template, css, user_template
...
def get_Word_File():
    word_Documents = st.file_uploader(
        label="Upload your word here",
        accept_multiple_files=True,
        type="docx",
        label_visibility="hidden",
    )
...
def get_txt_File():
    txt_Documents = st.file_uploader(
        label="Upload your txt here",
        accept_multiple_files=True,
        type="txt",
        label_visibility="hidden",
    )
...
def get_Audio_File():
    Audio_Files = st.file_uploader(
        label="Upload your Audio here", type=["mp3", "m4a", "wav"], label_visibility="hidden"
    )
...
def Word_text_preprocessing(Word_Documents):
    ...
    Word_Split = NLTKTextSplitter(
        chunk_size=500, chunk_overlap=500, separator="\n", length_function=len
    )
...
def Process_LLM(VectorStorage):
    ...
    LLM = ChatOpenAI(
        streaming=True, callback_manager=callback_manager, verbose=True, temperature=0.75
    )
...
def ConvoPlace(
    queries, ConversationModel, target_language, processor, model, speaker_embeddings, vocoder
):
...
def main():
    ...
    if (
        PDF_chunk is not None
        or Word_chunk is not None
        or Txt_chunk is not None
        or Audio_chunk is not None
    ):
        Combined_Chunks = []
    ...
    with st.sidebar:
        ...
        target_language = st.selectbox(
            "Select the language", ("English", "French"), label_visibility="hidden"
        )
